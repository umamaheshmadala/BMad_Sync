...
Deno.test("signup function - successful signup", async () => {
  ...
  const response = await handleSignup(mockRequest, mockSupabaseClient);
...
Deno.test("signup function - missing email or password", async () => {
  const mockRequest = new Request("http://localhost/signup", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
Deno.test("signup function - Supabase error", async () => {
  const mockSignUp = createMockFn<(_: SignUpWithPasswordCredentials) => Promise<{ data: { user: User | null, session: Session | null }, error: any }>>();
  mockSignUp.mockResolvedValueOnce({
  ...
  // Explicitly cast to access email property
  const credentials = mockSignUp.calls[0][0] as { email?: string };
  assert(credentials.email === "existing@example.com");
});